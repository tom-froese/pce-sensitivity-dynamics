...
def fit_cohort(centers: np.ndarray, E: np.ndarray, E_lo: np.ndarray):
    ...
    # Bootstrap the peak over participants.
...
def _print_report(subjects, fits, ct, click_mode, pairs, rho_pc):
    f = fits
    print(f"\n=== 1/f EXPONENT vs SENSITIVITY curve (noICA, {len(subjects)} participants) ===")
    print(f"  fits (R²):  sensitivity-peak[λ=e]={f['r2_sens']:+.3f}  "
          f"inverted-U(quad)={f['r2_quad']:+.3f}  R(x)-decay={f['r2_decay']:+.3f}  "
          f"trough={f['r2_trough']:+.3f}  line={f['r2_lin']:+.3f}")
    print(f"  free-λ sensitivity fit (fixed onset τ={TAU}s): "
          f"λ={f['lam_free']:.2f} (1/e-law predicts e={np.e:.2f}), R²={f['r2_free']:.3f}")
    print(f"  + BOOT-UP onset free: (λ=e) τ0={f['tau0_e']:.1f}s R²={f['r2_boot_e']:.3f} "
          f"peak={f['peak_boot_e']:.1f}s | (free λ) λ={f['lam_boot']:.2f} "
          f"τ0={f['tau0_boot']:.1f}s R²={f['r2_boot']:.3f} peak={f['peak_boot']:.1f}s  "
          f"[ΔR² vs fixed-onset = {f['r2_boot'] - f['r2_free']:+.3f}]")
    print(f"  PEAK / inflection time: argmax(mean)={f['argmax_t']:.1f}s; "
          f"quad-vertex={f['vertex']:.1f}s; free-λ peak={f['peak_free']:.1f}s; "
          f"bootstrap median={f['pk_med']:.1f}s 95%CI[{f['pk_lo']:.1f},{f['pk_hi']:.1f}]")
    print(f"  reference: 1/e t*={f['t_star']:.1f}s; "
    ...
    print(f"  => exponent peak CI {'BRACKETS' if brackets_1e else 'does NOT bracket'} "
          f"1/e ({t_star:.1f}s); "
          f"{'BRACKETS' if brackets_pas else 'does NOT bracket'} "
          f"the PAS/click point ({PAS_CROSSOVER_S:.0f}s)")
    survives_lo = f['r2_sens_lo'] > 0.5 and 18 <= f['argmax_lo'] <= 40
    print(f"  CONFOUND CHECK (EMG-reduced 2-20 Hz): "
          f"sensitivity-peak R²={f['r2_sens_lo']:+.3f}, "
    ...
    print(f"  PER-PARTICIPANT inverted-U: {100*(curv<0).mean():.0f}% have negative "
          f"curvature (peak), t-test curvature<0: t={tcurv.statistic:+.2f} "
          f"p={tcurv.pvalue:.1e} (n={len(curv)}); individual peak times "
          f"median={np.median(ppeak):.1f}s "
          f"[IQR {np.percentile(ppeak,25):.0f}-{np.percentile(ppeak,75):.0f}]")
    if rho_pc is not None:
        print(f"  ★ PER-PARTICIPANT exponent-peak vs OWN click time: "
              f"Spearman={rho_pc.correlation:+.3f} (p={rho_pc.pvalue:.3f}, "
              f"n={len(pairs)})  [POS => participants whose exponent peaks "
              f"later also click later = within-person sensitivity link]")
