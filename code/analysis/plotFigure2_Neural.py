...
Three panels:
  ...
  (A) — Global scalp potential (64-channel mean) grand median, full width,
             overlaid with the free-tau affine fit A*S(x) + B, with the signed
             sensitivity S(x) = -x*exp(-e*x) (the GSP is a trough, so A > 0).
  ...
  signed rate sensitivity S(x) = ∂R/∂λ = -x·e^{-λx} (a trough) — that used to be Panel A is
...
def fit_locked(t_sm, sm, tau=TAU_LOCKED):
    ...
    x = (t_fit - tau) / Teff
    s = -x * np.exp(-E * x)  # signed sensitivity S(x) = dR/dlambda (a trough)
...
trough_shape  = -(1.0 / E) * np.exp(-1.0)  # signed S(x) at x=1/e (a trough)
