...
text(0.97, 0.05, ...
    {sprintf('\\tau = %.0f s (locked)', best.offset), ...
     sprintf('R^2 = %.2f', best.R2), ...
     sprintf('peak = %.0f s', best.peak_time)}, ...
...
plot(tPlot, pPlot * 100, '-', 'Color', col_fit, 'LineWidth', 2, ...
    'DisplayName', sprintf('Exp. rise,  R^2 = %.2f', haptic_R2));
...
text(0.12, 0.95, sprintf('t_{95%%} \\approx %.0f s', haptic_t95), ...
    'Units', 'normalized', 'FontSize', font_sz_annot, ...
...
text(0.97, 0.95, ...
    {sprintf('\\tau = %.0f s (locked)', tau_eda_opt), ...
     sprintf('R^2 = %.2f', R2_eda_best), ...
     sprintf('A_0 = %.1f \\muS', A0_eda_best)}, ...
    'Units', 'normalized', 'FontSize', font_sz_annot, ...
